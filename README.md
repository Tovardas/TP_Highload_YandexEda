...
- * При отключении сервиса Stores, информация о ресторанах будет браться из заранее сохраненного кеша о 20 самых популярных (по количеству заказов за последний месяц) ресторанах.
  * При отключении сервиса Products, информация о продуктах будет браться из заранее сохраненного кеша о 15 самых популярных позициях (по количеству заказов за последний месяц) в каждом из сохраненных ресторанов. Сохраняться будет только ссылка на изображение, название и цена
  * При отключении сервиса Couriers, заранее сообщить, что доставка займет больше времени и точный интервал доставки будет известен позже.  
  * При отключении сервиса Cart, сохранять корзину в локальное хранилице, предупредив пользователей, что содержимое корзины может быть утеряно при выходе
  * При отключении сервиса GEO, на основе предыдущих доставок, каждому курьеру присваивается своя зона доставки, уведомление о новом заказе приходит половине курьеров в зоне. Заказ доставляет тот, кто принял заказ быстрее всего. Так как курьеры находятся в своей зоне, время доставки будет сохраняться в допустимых пределах. При попытке пользователя отследить свой заказ, вместо местонахождения курьрера, будут отображаться лишь стадии готовности заказа "Заказ готовиться, Заказ готов, Курьер в пути"
  * При отключении сервиса Orders, пользователь не сможешь смотреть свою историю заказов, а при попытке заказа, будет показываться сообщение о том, что в данный момент оформить заказ невозможно, попробуйте позже
  * При отключении сервиса Reviews, информация о рейтинге ресторанов и об отзывах будет сохранена в кеше (только о 20 самых популярных ресторанах), сохраняются только 100 самых актуальных комментариев. Предупредить пользователей, что написание отзыва временно недоступно
  * При отключении сервиса Search, будет исползоваться информация о 100 самых популярных запросах за месяц, сохраненная в кеше
...
| Сервис | Конфиг | Количество | Приблизительная стоимость | Приблизительная стоимость аренды |
| --- | --- | --- | --- | --- |
| Nginx            | 16 core CPU; 2x8 GB RAM 2400 MHz DDR4 | 2          | 120 тыс. руб | 15 тыс. руб           |
| Nginx            | 4 core CPU; 1x8 GB RAM 2400 MHz DDR4  | 10         | 47 тыс. руб  | 5 тыс. руб            |
| Auth Service      | 4 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 16         | 40 тыс. руб  | 3.5 тыс. руб          |
| User Service      | 4 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 16         | 40 тыс. руб  | 3.5 тыс. руб          |
| Search Service    | 2 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 16         | 27 тыс. руб  | 2 тыс. руб            |
| Cart Service      | 1 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 12         | 18 тыс. руб  | 1.5 тыс. руб          |
| Reviews Service   | 2 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 12         | 27 тыс. руб  | 2 тыс. руб            |
| GEO Service       | 2 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 12         | 27 тыс. руб  | 2 тыс. руб            |
| Products Service  | 2 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 16         | 27 тыс. руб  | 2 тыс. руб            |
| Stores Service    | 4 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 12         | 40 тыс. руб  | 3.5 тыс. руб          |
| Orders Service    | 4 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 12         | 40 тыс. руб  | 3.5 тыс. руб          |
| API Gateway       | 16 core CPU; 1х4 GB RAM 2400 MHz DDR4 | 18         | 115 тыс. руб | 15 тыс. руб           |

Суммарная стоимость при покупке:

240 000 + 470 000 + 640 000 + 640 000 + 432 000 + 216 000 + 324 000 + 324 000 + 432 000 + 480 000 + 480 000 + 2 070 000 = 6 748 000 руб

Суммарная стомость аренды за месяц:

30 000 + 50 000 + 56 000 + 56 000 + 32 000 + 18 000 + 24 000 + 24 000 + 32 000 + 42 000 + 42 000 + 270 000 = 676 000 руб

Таким образом, за год стоимость аренды составит 8 112 000 руб -> При покупке оборудования, вложения окупятся менее, чем за год.
