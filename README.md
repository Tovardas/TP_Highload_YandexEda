...
Для быстрого поиска используется инверсивное индексирвоание данных ресторанов, блюд, магазинов и продуктов. Индексы создаются заранее и обновляются в реальном времени при изменениях (добавление нового контента). Поиск ведется по всем атрибутам, т.е. по названию блюд, по названию ресторанов, по названию магазинов или названию продуктов.
...
Пользователь видит список релевантных ресторанов и блюд

**Распределение нагрузки по предприятиями (Least Connectioins)**

1. Инициализация

Балансировщик получает информацию о текущей загрузке всех ресторанов и курьеров. В данном случае «загрузка» измеряется количеством активных заказов

2. Выбор оптимального предприятия

Когда поступает новый заказ, балансировщик проверяет, у какого ресторана из соответствующей зоны на данный момент меньше всего активных задач. 
Заказ передается туда, где в данный момент меньшая загрузка (меньше всего активных заказов). Если нагрузка (кол-во заказов) одинаковая, выбирается ближайшее к адресу доставки предприятие.

3. Обновление статуса

После того как заказ передан ресторану или курьеру, система увеличивает их счетчик активных заказов.
Как только заказ будет выполнен, счетчик уменьшается.

**Распределение курьеров по заказам**

1. Получение заказа

Когда клиент оформляет заказ, система регистрирует информацию о месте доставки, ресторане, содержимом заказа и времени ожидания

2. Проверка доступных курьеров

Система ищет всех активных курьеров в радиусе зоны доставки

3. Оценка курьеров с помощью алгоритмов распределения

Выбирается курьер с помощью RoundRobin

4. Определения траспортного средства

Определяется транспортное средство, доступное курьеру

5. Построение маршрута

С учетом траспортного средства, инфраструктуры и т.п. строится маршрут до Предприятия и до Адреса доставки. Для оптимизации маршрута исопльзуется алгоритм Дийкстры. Потенциальные пути для курьера предсталяются в виде графа и ищется кротчайший путь. Граф взвешенный и весом ребер является время в дороге по определенному участку пути.

6. Мониторинг выполнения

Система отслеживает реальное местоположение курьера с помощью GPS и обновляет маршрут в реальном времени, если изменились условия, например, пробки

7. Завершение заказа
   
После завершения доставки система обновляет статус курьера (порядок в очереди) и проверяет его готовность принять следующий заказ
...
| **Webpack**              | Сборка модулей фронтенда                                           | Удобство для SPA, увеличение скорости загрузки, предупреждение ошибок                         |
