...
**Входные данные:**
Примерное количество запросов: 700 запросов в секунду.
Средний размер каждого запроса: 30 КБ.
**Допущения:**
Алгоритм шифрования RSA или ECDSA.
Стандартный срок жизни сессии и пересчет ключей (обновления) происходит каждые 1–2 минуты.
##### Расчет нагрузки:
**Общий объем данных:**
700 запросов * 30 КБ = 21 МБ/сек данных.

**Ресурсы на терминацию SSL:**
Для RSA - требователен к процессору, так как использует большие ключи (1024, 2048 бит). Процессор может обрабатывать 2000-3000 таких операций за секунду на одно ядро.

Для ECDSA — более эффективен, использует меньшие ключи, что требует меньше вычислительных ресурсов, повышая производительность (больше сессий в секунду), но имеет меньшую защищенность.
